...
In instances where simulation software is unavailable for researching the parameter $k$ in the measuring circuit, the attached graph of Figure 3 serves as a valuable tool for estimating it according to the distance $s$ between the grounding elements. This estimation is especially pertinent in cases where the grounding configuration includes turbine foundations in the cylindrical format, constructed with reinforced concrete, and horizontal electrodes.
